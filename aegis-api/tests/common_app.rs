...
pub async fn build_test_app(pool: PgPool, redis: RedisClient) -> anyhow::Result<TestApp> {
    ...
    // Wrap with the same security layer used in main.rs so tests exercise the
    // hardened router (and the security_headers_test asserts on it).
    let router = security_layer::apply(inner);
